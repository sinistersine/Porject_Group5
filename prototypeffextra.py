...
with tab_visuals:
    ...
    if uploaded_file:
        ...
        # --- hard remove oude widgets door state te wissen (voor het geval ze ooit bestonden)
        for k in ("group_by_radio", "cap_kwh_visual", "start_soc_visual"):
            if k in st.session_state:
                del st.session_state[k]

        # --- vaste settings, geen UI ---
        group_by_display = "Bus"
        group_by = "bus"      # <-- zorg dat dit exact je kolomnaam is in df
        cap_kwh = 300.0
        start_soc = 100

        # optioneel filteren op specifieke bussen
        ...
        if not under10.empty:
            ...
            st.error(
                "⚠️ Batterij < 10% — bus(sen) niet beschikbaar: "
                + ", ".join(hits[group_by].astype(str).tolist())
            )
            with st.expander("Eerste moment <10% per bus"):
