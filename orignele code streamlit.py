...
def plot_gantt_chart(uploaded_file):
    ...
    fig = px.timeline(
        ...
        color_discrete_map={
            "service trip": "#377eb8",   # roze
            "material trip": "#ff7f00", # lichtroze
            "idle": "#4daf4a"           # pastel roze
        }
    )
...
def energy_consumption(uploaded_file):
    # Lees Excel in
    df = pd.read_excel(uploaded_file, sheet_name="Sheet1")

    # Filter alleen rijen met een geldige buslijn
    df_filtered = df[df['line'].notna()]

    # Groepeer op buslijn en sommeer energieverbruik
    energy_per_line = df_filtered.groupby('line')['energy consumption'].sum().reset_index()

    # Maak interactieve staafdiagram
    fig = px.bar(
        energy_per_line,
        x='line',
        y='energy consumption',
        labels={'line': 'Buslijn', 'energy consumption': 'Energieverbruik (kWh)'},
        title='Totale energieverbruik per buslijn',
...
with tab_visuals:
    ...
    if uploaded_file:
        fig = energy_consumption(uploaded_file)
        st.plotly_chart(fig, use_container_width=True)
    else:
        st.info("Upload een Excel-bestand in de sidebar om de visualisaties te zien.")
